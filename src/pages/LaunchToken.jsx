...
export default function LaunchToken({ setPage }) {
  ...
  async function handleLaunch() {
    ...
    try {
      ...
      setStepMsg('1/4 — Upload metadata token...')
      ...
      // Step 2: Ambil config key
      setStepMsg('2/4 — Ambil config...')
      ...
      setStepMsg('3/4 — Membuat transaksi...')
      ...
      // Step 4: Sign dengan wallet
      setStepMsg('4/4 — Silahkan sign di wallet kamu...')
  ...
  if (!publicKey) return (
    <div className="max-w-xl mx-auto px-4 py-20 text-center">
      ...
      <p className="text-white/50 mb-6 text-sm">Butuh wallet Solana (Phantom/Solflare) untuk launch token</p>
      <WalletMultiButton />
    </div>
  )
  ...
  if (step === 'done' && result) return (
    <div className="max-w-xl mx-auto px-4 py-16 text-center">
      ...
      <div className="p-5 rounded-xl bg-white/5 border border-white/10 text-left space-y-3 mb-8">
        <div className="flex justify-between"><span className="text-white/50 text-sm">Token</span><span className="text-white font-medium">{result.name} (${result.symbol})</span></div>
        <div className="flex justify-between"><span className="text-white/50 text-sm">Mint</span><span className="text-white/70 text-xs font-mono">{result.tokenMint?.slice(0,16)}...</span></div>
        <div className="flex justify-between"><span className="text-white/50 text-sm">Solscan</span>
          <a href={`https://solscan.io/tx/${result.signature}`} target="_blank" rel="noopener noreferrer" className="text-emerald-400 text-xs hover:underline">Lihat transaksi →</a>
        </div>
        <div className="flex justify-between"><span className="text-white/50 text-sm">Bags.fm</span>
          <a href={`https://bags.fm/${result.tokenMint}`} target="_blank" rel="noopener noreferrer" className="text-emerald-400 text-xs hover:underline">Lihat di Bags →</a>
        </div>
      </div>
      <div className="flex gap-3 justify-center">
        <button onClick={() => { setStep('form'); setForm({ name:'', symbol:'', description:'', twitter:'', website:'', initialBuy:'0.1' }); setImageFile(null); setImagePreview('') }}
          className="px-5 py-2.5 rounded-xl border border-white/20 text-white/70 text-sm hover:bg-white/5">Launch lagi</button>
        <button onClick={() => setPage('dashboard')} className="px-5 py-2.5 rounded-xl bg-emerald-500 text-white text-sm font-medium hover:bg-emerald-600">Lihat Dashboard →</button>
      </div>
    </div>
  )
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-8">
      ...
      {error && <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-sm mb-6 break-all">⚠️ {error}</div>}
      {step === 'launching' && (
      ...
      <div className="space-y-5">
        <div>
          <label className="block text-sm text-white/60 mb-2">Gambar Token</label>
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden flex-shrink-0">
              {imagePreview ? <img src={imagePreview} alt="preview" className="w-full h-full object-cover" /> : <span className="text-white/20 text-2xl">+</span>}
            </div>
            <label className="cursor-pointer px-4 py-2 rounded-lg border border-white/20 text-white/60 text-sm hover:bg-white/5">
              Pilih gambar<input type="file" accept="image/*" onChange={handleImage} className="hidden" />
            </label>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input name="name" value={form.name} onChange={handleInput} placeholder="Juragan Bear" maxLength={32}
          ...
          <div>
            ...
            <input name="symbol" value={form.symbol} onChange={handleInput} placeholder="JBEAR" maxLength={10}
              className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm outline-none focus:border-emerald-500/50" />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea name="description" value={form.description} onChange={handleInput} placeholder="Ceritakan tentang token kamu..." rows={3} maxLength={1000}
        ...
        <div>
          ...
          <input name="initialBuy" type="number" step="0.001" min="0.001" value={form.initialBuy} onChange={handleInput}
            className="w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm outline-none focus:border-emerald-500/50" />
          <p className="text-xs text-white/30 mt-1">Jumlah SOL yang kamu beli saat launch</p>
        </div>

        <button onClick={handleLaunch} disabled={!form.name || !form.symbol || !form.description || step === 'launching'}
